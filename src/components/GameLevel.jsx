...
const GameLevel = ({
  ...
  // Hook de confeti
  const { isActive: isConfettiActive, triggerConfetti: triggerConfettiEffect } =
    useConfetti();
  ...
  const handleAnswer = (optionIndex) => {
    ...
    if (correct) {
      ...
      playSuccess(); // ✅ Sonido de respuesta correcta
  ...
  const handleWatchVideo = (isBeforeStart = false) => {
    ...
    const videoUrl = {
      ...
      4: "https://youtu.be/oB4ol3EYEIs",
      5: "https://youtu.be/yUOPtM_yEHc",
      6: "https://youtu.be/NMY7SwEgC50",
      7: "https://youtu.be/TU9VF7sWh-w",
      8: "https://youtu.be/7yo2d-MtgBw",
      9: "https://youtu.be/rTJXMJqkUSw",
      10: "https://youtu.be/pem8VkjbNA4",
      11: "https://youtu.be/iJNKzYCRoEw",
      12: "https://youtu.be/VvHqG2ec744",
      13: "https://youtu.be/scs2OI7IB2c",
      14: "https://youtu.be/HBnbjZKqyjA",
      15: "https://youtu.be/5KbUJ6fDVRA",
      16: "https://youtu.be/DVlSigTdaoQ",
      17: "https://youtu.be/e0AbDC1DlLI",
      18: "https://youtu.be/pvOTsv6alS8",
      19: "https://youtu.be/p2qaUIymS9M",
      20: "https://youtu.be/w_YbsjGtn1s",
      21: "https://youtu.be/hvI5afYV9kM",
    }[topic.id];

    console.log(
      `🎥 handleWatchVideo ejecutado - Módulo: ${topic.id}, URL:`,
      videoUrl,
    );
  ...
  const handleAbandon = () => {
    // Resetear el estado del juego antes de salir
    setCurrentFloor(0);
    setSelectedOption(null);
    setIsCorrect(null);
    setCompleted(false);
    setLives(5);
    setScore(0);
    // Llamar a onExit para cerrar el nivel
    onExit();
  };
      ...
      >
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl max-w-md w-full relative z-10">
          ...
          <p className="text-slate-300 mb-6 text-sm leading-relaxed">
            Completa todas las preguntas correctamente y demuestra tu dominio en
            este módulo.
          </p>

          {/* Video Badge */}
          <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-4 mb-6">
            <p className="text-cyan-300 font-black text-sm mb-3">
              📺 VIDEO DE REPASO DISPONIBLE
            </p>
            <button
    ...
    >
      ...
      <div className="bg-slate-900/80 backdrop-blur-md p-4 flex justify-between items-center z-50 border-b border-white/10">
        <button
          ...
          className="text-slate-400 hover:text-white flex items-center gap-2 font-bold transition-colors uppercase tracking-wider text-xs"
        >
          <ArrowLeft size={16} />{" "}
        ...
        <div
          className={`flex items-center gap-1.5 transition-all ${loseHeartAnimation ? "animate-shake" : ""}`}
        >
          {[...Array(5)].map((_, i) => (
            <div key={i} className="relative">
              {i < lives ? (
                <span className="text-xl drop-shadow-[0_0_10px_rgba(239,68,68,0.8)]">
                  ❤️
                </span>
              ) : (
          ...
          <div className="bg-slate-800 border border-white/10 rounded-lg overflow-hidden shadow-lg flex">
            {getItemCount("extra_life") > 0 && (
              <button
                onClick={() => {
                  if (useConsumable("extra_life")) {
                    setLives(
                      Math.min(lives + 1, hasUpgrade("resistance_1") ? 6 : 5),
                    );
                  }
                }}
              ...
              >
                ...
                <span className="text-xs ml-1 font-black text-cyan-300">
                  {getItemCount("extra_life")}
                </span>
              </button>
            )}
      ...
      <div className="flex-1 flex items-center justify-center relative px-4 py-12">
        <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none">
          <div className="w-full max-w-2xl h-96 rounded-3xl border-8 border-cyan-500/30 flex">
            <div
              className={`flex-1 bg-gradient-to-r from-slate-800 to-slate-900 border-r-4 border-black transition-transform duration-700 ease-in-out ${isDoorOpening ? "-translate-x-full" : "translate-x-0"}`}
            ></div>
            <div
              className={`flex-1 bg-gradient-to-l from-slate-800 to-slate-900 border-l-4 border-black transition-transform duration-700 ease-in-out ${isDoorOpening ? "translate-x-full" : "translate-x-0"}`}
            ></div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-slate-900 border-2 border-cyan-500/50 w-full max-w-2xl rounded-3xl p-0 shadow-[0_0_80px_rgba(34,211,238,0.3)] animate-in zoom-in-95 duration-300 overflow-hidden">
              ...
              <div className="p-4 flex justify-between items-center">
                ...
                >
                  {[...Array(5)].map((_, i) => (
                    <span
                      key={i}
                      className={`text-xl drop-shadow-[0_0_8px_rgba(239,68,68,0.6)] transition-all ${i < lives ? "scale-100" : "scale-75 opacity-40"}`}
                    >
                      {i < lives ? "❤️" : "💔"}
                    </span>
                  ))}
            ...
            <div className="p-8">
                ...
                {topic.questions[currentFloor].options.map((opt, idx) => (
                  <button
                    ...
                    style={{
                      animation:
                        selectedOption === null
                          ? `slide-in-left 0.3s ease-out ${idx * 50}ms both`
                          : "none",
                    }}
                  ...
                  >
                    <div className="relative z-10 flex items-center gap-5">
                      <div
                        className={`w-10 h-10 rounded-lg border-2 flex items-center justify-center text-sm font-black flex-shrink-0 transition-all ${
                          selectedOption === idx
                            ? "border-current bg-current/10"
                            : "border-slate-500 text-slate-400 group-hover:border-cyan-400 group-hover:text-cyan-400 group-hover:bg-cyan-400/10 group-hover:scale-110"
                        }`}
                      >
              ...
              {selectedOption !== null && (
                <div
                  className={`mt-6 p-4 rounded-lg text-center font-black text-sm tracking-widest uppercase animate-in slide-in-from-bottom-2 transition-all ${
                    isCorrect
                ...
                >
                  <div
                    className={`text-lg mb-2 transition-all ${isCorrect ? "animate-bounce" : "animate-shake"}`}
                  >
                  ...
                  <div className="text-xs text-slate-400 mt-3 opacity-75">
                    Siguiente pregunta en 1.5s...
                  </div>
                </div>
              )}
            </div>
